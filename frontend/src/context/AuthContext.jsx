...
export function AuthProvider({ children }) {
  ...
  const updateProfile = async (updatedFields) => {
    ...
    setRegisteredAccounts(prev =>
      prev.map(acc => (acc.email.toLowerCase() === updatedUser.email.toLowerCase() ? updatedUser : acc))
    );

    try {
      await fetch(`http://localhost:8081/api/users/profile?email=${encodeURIComponent(updatedUser.email)}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(updatedUser)
      });
    } catch (e) {
      console.warn('Backend profile update sync skipped/failed:', e);
    }
  };

  return (
    <AuthContext.Provider value={{ user, token, isAuthenticated: !!user, registeredAccounts, login, register, logout, updateProfile }}>
      {children}
